...
*ls* - **показывает все файлы и каталоги**

*cd <название нужной папки>* - **перемещается в нужную нам для работы папку**
...
В результате на экран выводится следующее: [Пример](https://gist.github.com/Jekins/2bf2d0638163f1294637).
...
В результате на экран выводится следующее:
    ...
    [пример] [id] [id]: https://gist.github.com/Jekins/2bf2d0638163f1294637 "Необязательная подсказка". 
...
Обратный слеш может употребляться в Markdown перед специальными символами для того, чтобы они воспринимались в их буквальном (а не служебном) значении. Полный список данных символов приводится ниже:
...
В результате на экран выводится следующее: <ineideot@mail.ru>
...
*✨P.S. Информация постепенно пополняется.✨*

##### *P.P.S. Посмотрел "*git log --graph*" и понял что перемудрил маленько.*

Получается, что каждую новую ветку нужно создавать из головной ветки(master)?